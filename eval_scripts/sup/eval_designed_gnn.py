...
if __name__ == "__main__":
    ...
    gnn_list = [
        ...
        # [0, 'gat_6', 0, 'gcn', 0, 'gcn', 2, 'arma']: cells_info
        # 0, 0, 0, 2: index
        {'action': [0, 'gcn', 0, 'sage', 1, 'arma', 'sigmoid', 'add'], 'hyper_param': [0.005, 0.8, 1e-05, 64]},
        {'action': [0, 'linear', 0, 'gat_4', 2, 'linear', 'relu', 'concat'], 'hyper_param': [0.01, 0.5, 1e-05, 128]},
        {'action': [0, 'linear', 0, 'gat_2', 2, 'sg', 'elu', 'concat'], 'hyper_param': [0.01, 0.5, 1e-05, 128]},
        # {'action': [0, 'gat_6', 0, 'gcn', 0, 'gcn', 2, 'arma', 'tanh', 'concat'], 'hyper_param': [0.01, 0.9, 0.0001, 64]},
    ...
    for shuffle in [False, True]:
        for dataset, actions in zip(dataset_list, gnn_list):
            with open("/home/yc568/GraphNAS/datacache1.txt", mode='a') as file:
                file.write("starting " + dataset + " here, shuffle = " + str(shuffle) + '\n')
                file.close()
            ...
            if shuffle:
                with open("/home/yc568/GraphNAS/datacache1.txt", mode='a') as file:
                    file.write("finished " + dataset + " here\n")
                    file.close()
